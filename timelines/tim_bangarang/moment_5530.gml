		...
		targetY = lerp(0, 670, 0.5)
		em = quickEmitter(400, targetY, obj_bulletPlayer.x, obj_bulletPlayer.y, 16, 1, 1, 360/16, "", 1)
time_source_reconfigure(obj_gmcrtl.onbeat, BANGARANG_CONTROL.secondsBeat, time_source_units_seconds,function()
{
			em = quickEmitter(400, targetY, obj_bulletPlayer.x, obj_bulletPlayer.y, 1, 1, 1, 360, "", 1,,,,obj_bulletBounce) //this works?
			//YES IT DOES
}, [], -1);
...
shootLeft = function(){
	var r = quickLazer(room_width/3.8, 0, room_width/3, 0, 1, "", 60,obj_lazer, make_color_rgb(0,0,255))
	r.image_xscale = 3
}
shootRight = function(){
	var f = quickLazer(room_width/1.2, 0, room_width/1.5, 0, 1, "", 60, obj_lazer, make_color_rgb(255,0,0))
	f.image_xscale =3
}
...
{
	em = quickEmitter(400, targetY, obj_bulletPlayer.x, obj_bulletPlayer.y, 2, 1, 1, 90/2, "", 5)
}, [], -1);
