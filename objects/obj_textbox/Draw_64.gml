if (drawthings) { //big ahh if

//Draw textbox
#region Draw name and namebox
var cname = name[page]
var csprite = speaker[page].myPortrait
if(cname != "None"){
	//Draw namebox
	draw_sprite_ext(name_box, 0, name_box_x ,name_box_y, scale,scale, 0, c_white, 1); //name box
...
#endregion
draw_sprite_ext(dialogue_box, 0, pos_x - offsets*24,pos_y, scale + 1,scale, 0, c_white, 0.9);
...
if characterSpr[page] != spr_box { //-496
	draw_sprite_ext(characterSpr[page], emotion[page], pos_x-portraitWidth, pos_y - 496 + boxHeight/*boxHeight*/, 2,2, 0, c_white, 1);
	}
	else { //496 == spr height
	draw_sprite_ext(csprite, emotion[page], pos_x-portraitWidth, pos_y - 496 + boxHeight, 2,2, 0, c_white, 1);

}
	#region Idle Animated Portrait /////////////////////// hereee
if(type[page] == 1 or charCount >= str_len) {
	if(portrait_idle[page] != -1){
		...
		if(portrait_idle_c >= portrait_idle_n[page]){ portrait_idle_c = 0; } //boxheight
		//draw_sprite_ext(portrait_idle[page], portrait_idle_c, posx, posy - boxHeight, scale,scale, 0, c_white, 1);	
	}
}
#endregion
...
#region TYPE 0: NORMAL DIALOGUE
	...
	if(charCount < str_len and !pause){
		...
		if(text_speed_c+1 < text_speed_al and charCount == txtspd[tsc2+2]) {
			text_speed_c++;
			tsc2 = (text_speed_c*2)*global.textSpeedMult //here?
		}
		//important
		...
		switch(ch){
			...
			case ".":
				pause = true;
				alarm[1] = 20/global.textSpeedMult;	//how many frames we wait if we detect a fullstop or comma
				break;
			
			case "?":
			case "!":
				pause = true;
				alarm[1] = 20/global.textSpeedMult;	//how many frames we wait if we detect a ! or ?
				break;
			default:
	...
	repeat(charCount){
		...
		switch(effect){
			...
			case 2:	//wave
				var so = t;
				var shift = sin(so*pi*freq/room_speed)*amplitude; //learn params
				draw_text_color(xx + (cx*charSize), yy+(cy*stringHeight)+shift, letter, col, col, col, col, 1);
				break; 
			
			case 3: //colour shift
				var c1 = make_colour_hsv(t+cc, 255, 255); //rainbow
