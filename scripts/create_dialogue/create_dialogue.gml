...
///@arg Text
///@arg Speaker //CHARACTER IMG OR ID I THINK?
///@arg *Effects
///@arg *Speed
///@arg *Type
///@arg *Next_Line
///@arg *Scripts
///@arg *Text_Col
///@arg *Emotion //FRAMEEEEE
///@arg *Emote
///@arg *Sprite
//delay is 4 intregrated everywhere here now its how long to wait
function create_dialogue() {
	...
	repeat(arg_count){ arg[i] = argument[i]; i++; } //learn copypaste this
	...
	var _textcol	= array_create(text_len, [1,c_white]); //lmaooo ok
    ...
    var _sprite	= array_create(text_len, spr_box);
	var _emotes		= array_create(text_len, -1);
	var _creator	= array_create(text_len, id);
	var _position = array_create(text_len, -1); //-1 for left or 1 for right
	//var _delay      = array_create(text_len, 1); //will creator fuck me up?

	var a;
	//Fill variables depending on argument count
	switch(arg_count-1){ //smartu shitu
		case 10: a = arg[10]; if(array_length_1d(a) != text_len){ a[text_len] = 0; } for(i = 0; i < text_len; i++){ if(a[i] != 0) _sprite[i] = a[i]; }
	...
	with(_textbox){
		...
		text_col	= _textcol;
		emotion		= _emotion;	//FRAME
		emotes		= _emotes;
		characterSpr      = _sprite
